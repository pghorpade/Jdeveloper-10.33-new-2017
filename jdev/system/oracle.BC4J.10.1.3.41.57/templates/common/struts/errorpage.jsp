<%@ page language = "java" isErrorPage="true" import = "java.util.*, java.io.*, oracle.jbo.*, oracle.jdeveloper.html.*, oracle.jbo.html.databeans.*, org.apache.struts.action.*" contentType="text/html;charset={%/PageWizard/charset%}" %>
...
<% // This page is also use to display errors from the Struts exception handler
   if (exception == null)
      exception = (Exception) request.getAttribute(Action.EXCEPTION_KEY);
%>

<center><H2><bean:message key="errorpage.apperror"/></H2></center>

<div CLASS="errorText">
<bean:message key="errorpage.message"/> <%= exception.getMessage() %>
<BR>
<%
...
%><H3>&nbsp;&nbsp;&nbsp;&nbsp;<bean:message key="errorpage.message"/> <%=e.getLocalizedMessage()%></H3>
<BR><%
      }
   } %>
